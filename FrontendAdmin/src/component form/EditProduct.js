...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get("https://localhost:7192/api/category");
    ...
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`https://localhost:7192/api/product/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `https://localhost:7192/api/product/${product.id}`,
        formData,
        {
          headers: {
  ...
  return (
    <div className="container max-w-3xl p-6 mx-auto mt-10 bg-white shadow-xl rounded-xl">
      <h2 className="mb-6 text-2xl font-bold text-center">Chỉnh sửa sản phẩm</h2>

      {error && <div className="mb-4 text-center text-red-600">{error}</div>}

      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-lg">Tên sản phẩm</label>
          <input
            type="text"
            name="name"
            value={product.name}
        ...
        <div>
          <label className="block text-lg">Giá</label>
          <input
            type="text"
            name="price"
            value={product.price}
        ...
        <div>
          <label className="block text-lg">Danh mục</label>
          <select
            name="categoryId"
        ...
        <div>
          <label className="block text-lg">Số lượng</label>
          <input
            type="text"
            name="quantity"
        ...
        {product.imageUrl && (
          <div>
            ...
            <img
              ...
              alt={product.name}
              className="object-cover w-16 h-16 mb-4 rounded-lg"
            />
          </div>
        )}
